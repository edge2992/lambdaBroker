...
```bash
software$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
software$ sam local start-api
software$ curl http://localhost:3000/
```
...
```bash
software$ sam logs -n HelloWorldFunction --stack-name software --tail
```
...
```bash
software$ cd HelloWorldFunction
...
```bash
sam delete --stack-name software
```
